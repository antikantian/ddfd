...
using ddfd_net = loss_multiclass_log<
		fc<2,
		relu<dropout<fc<4096,
		relu<dropout<fc<4096,
		max_pool<3,3,2,2,
...
int main(int argc, char** argv) try
{
	...
	dnn_trainer<ddfd_net> trainer(net,sgd(weight_decay, momentum));

	trainer.be_verbose();
	trainer.set_learning_rate(initial_learning_rate);
	trainer.set_synchronization_file("ddfd_trainer_state_file.dat", std::chrono::minutes(10));
	...
	softmax<ddfd_net::subnet_type> snet; snet.subnet() = net.subnet();
